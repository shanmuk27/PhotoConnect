...
class CategoryAdapter(
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        if (position == 0) {
            holder.bind(holder.itemView.context.getString(R.string.all_categories), selectedService == null) { onSelect(null) }
        } else {
            val index = position - 1
            val serviceType = serviceTypes[index]
            holder.bind(labels[index], selectedService == serviceType) { onSelect(serviceType) }
        }
    }
