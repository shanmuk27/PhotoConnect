...
data class EditablePostImage(
    val uri: Uri,
    val existingImageId: Int? = null,
)

class SelectedPostImageAdapter(
    private val onRemove: (Int) -> Unit,
) : RecyclerView.Adapter<SelectedPostImageAdapter.VH>() {
    private val items = mutableListOf<EditablePostImage>()

    fun submitList(images: List<EditablePostImage>) {
        items.clear()
        items.addAll(images)
    ...
    inner class VH(private val b: ItemSelectedPostImageBinding) : RecyclerView.ViewHolder(b.root) {
        fun bind(item: EditablePostImage) {
